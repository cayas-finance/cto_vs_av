...
from .results import SuccessionPEAResult


class SuccessionPEA(SuccessionBase):
    def compute(
        self,
        sim,
        autres_biens,
        ...
        notary_fees = 0.0
        if is_donation:
            base_pea_par_benef = max(0, (sim.capital / nb_benef) - abattement)
            dmtg_pea = calcul_impot_progressif(base_pea_par_benef, bareme) * nb_benef
            notary_fees = calcul_emoluments_notaire(sim.capital)
            succession_restante = max(0.0, droits_totaux - dmtg_pea)
            net_heir_total = (
                (sim.capital - dmtg_pea - notary_fees)
                + (autres_biens - succession_restante)
            )
            tax_attributable_pea = dmtg_pea
        else:
            net_heir_total = sim.capital + autres_biens - droits_totaux
            tax_attributable_pea = droits_totaux - droits_others

        net_contract_only = sim.capital - tax_attributable_pea - notary_fees

        return SuccessionPEAResult(
            ...
            tax_attributable_pea=tax_attributable_pea,
            droits_others=droits_others,
            notary_fees=notary_fees,
            pea_dmtg_donation=dmtg_pea,
        )
