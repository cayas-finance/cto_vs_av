...
def simulate_complex_diversified_portfolio():
    ...
    # NTSG, MF, Or
    corrs = np.array([[1.0, -0.1, 0.1], [-0.1, 1.0, 0.2], [0.1, 0.2, 1.0]])
    ...
    for av, cto in zip(heritages_av, heritages_cto):
        ...
        d = (total_av - total_cto) / m * 100 if m > 0 else 0
        diffs_rel.append(d)
    ...
    print("=== Portefeuille complexe (80% NTSG, 10% MF, 10% Or) ===")
    ...
    plt.axvline(
        ...
        linestyle="-",
        linewidth=2,
        label=f"Médiane ({median_diff_rel:+.2f}%)",
    )

    plt.title(
        "Distribution de l'avantage relatif (AV vs CTO)\n"
        f"Stratégie diversifiée 30 ans | Prob AV > CTO : {prob_av:.2%}"
    )
