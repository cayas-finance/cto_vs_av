...
class SimulationRequest(BaseModel):
    ...
    rendement: float = Field(
        default=0.05,
        description=(
            "Le rendement annuel moyen espéré (net de frais de fonds, brut de frais "
            "d'enveloppe pour CTO, net de frais de gestion UC pour AV si spécifié ainsi)."
        ),
    ...
    autres_biens: float = Field(
        ...
        ge=0,
        description=(
            "La valeur nette des autres biens entrant dans la succession (pour le calcul "
            "des droits marginaux)."
        ),
    )
    ...
    relation: str = Field(
        ...
        description=(
            "Le lien de parenté pour le barème successoral. Valeurs : 'ligne_directe', "
            "'frere_soeur', 'neveu_niece', 'sans_lien'."
        ),
    ...
    age_souscription: int = Field(
        ...
        ge=0,
        description=(
            "Âge de l'épargnant à la souscription (pour déterminer la fiscalité AV 70 ans)."
        ),
    )
    ...
    is_withdrawal_net: bool = Field(
        default=False,
        description=(
            "Si True, le montant du retrait visé est NET d'impôt (dans la poche). Si "
            "False, c'est un retrait BRUT (avant impôt)."
        ),
    )
    cto_is_donation: bool = Field(
        default=False,
        description=(
            "Si True, modélise une donation du CTO (DMTG appliqués sur le CTO, succession "
            "sur les autres biens uniquement)."
        ),
    )
...
class DetailedMetrics(BaseModel):
    ...
    gross_capital: float = Field(
        description="Capital brut du contrat avant fiscalité de sortie ou succession."
    )
    total_fees: float = Field(
        description="Total des frais de gestion/garde payés durant la vie du contrat."
    )
    
    # Succession Setup
    succession_gross: float = Field(
        description="Valeur brute de la succession avant impôts (contrat + autres biens)."
    )
    taxable_base_succession: float = Field(
        description=(
            "Assiette soumise aux droits de succession (après abattements spécifiques AV "
            "si applicables, ou brut purge pour CTO)."
        )
    )
    
    # Taxes & Net Heir (Contract Only)
    net_heir_contract_only: float = Field(
        description=(
            "Montant net perçu par l'héritier provenant UNIQUEMENT du contrat (fiscale "
            "marginale déduite)."
        )
    )
    
    # Global Scenario (Contract + Other Assets)
    scenario_tax_total: float = Field(
        description=(
            "Droits de succession TOTAUX payés dans ce scénario (sur Contrat + Autres "
            "Biens)."
        )
    )
    scenario_net_heir_total: float = Field(
        description=(
            "Total net final perçu par les héritiers (Contrat Net + Autres Biens Net)."
        )
    )
    total_tax_on_withdrawals: float = Field(
        default=0.0,
        description=(
            "Impôts totaux payés sur les plus-values lors des rachats durant la vie du "
            "contrat."
        ),
    )
    
    # AV Specifics
    av_ps_total: Optional[float] = Field(
        default=None,
        description="Prélèvements sociaux totaux payés lors du décès (AV uniquement).",
    )
    av_rights_total: Optional[float] = Field(
        default=None,
        description="Droits de succession (ou taxe 990I) payés sur la part AV.",
    )
    cto_dmtg_donation: Optional[float] = Field(
        default=None,
        description="DMTG appliqués au CTO en cas de donation.",
    )
    notary_fees: Optional[float] = Field(
        default=None,
        description="Frais de notaire (émoluments) estimés pour la donation CTO.",
    )
...
class SimulationResult(BaseModel):
    net_cto: float = Field(
        description=(
            "Valeur nette finale héritée via CTO (après droits de succession et retraits)."
        )
    )
    net_av: float = Field(
        description=(
            "Valeur nette finale héritée via AV (après droits de succession et retraits)."
        )
    )
    advantage_amount: float = Field(
        description="Montant de l'avantage financier de la meilleure enveloppe."
    )
